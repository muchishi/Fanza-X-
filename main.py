...
from config import FANZA, DOUJIN, POST_SCHEDULE, LOG_DIR, DATA_DIR
...
from poster.x_poster import post_item, download_image
...
def task_draft(force: bool = False):
    """
    X API を使わず、投稿予定の内容をローカルフォルダに書き出す半自動投稿モード。

    X APIの投稿(POST /2/tweets)が有料プラン必須になった環境向け。
    本文・リプライ文・添付予定のメディア（動画 or サンプル画像）を
    data/drafts/<queue_id>_<product_id>/ に出力するので、
    ユーザーがXアプリ/Webから手動でコピペ投稿する。

    手動投稿が終わったら `python main.py --confirm-posted <queue_id>` を実行し、
    post_log に記録する（再投稿クールダウン・実績集計を機能させるため）。
    """
    conn = get_conn()
    ...
    mark_draft(conn, item_data["id"], str(draft_dir))
    conn.close()
...
def confirm_posted(queue_id: int):
    """
    半自動投稿モードで手動投稿し終えた下書きを post_log に記録する。
    tweet_id/reply_tweet_id は手動投稿のため 'manual_<timestamp>' を使う。
    """
    conn = get_conn()
    item_row = get_draft_queue_item(conn, queue_id)
    if not item_row:
        print(f"draft状態のqueue_id={queue_id}が見つかりません。")
        conn.close()
        return None
    ...
    mark_posted(
        ...
        queue_id       = queue_id,
        tweet_id       = manual_id,
        reply_tweet_id = manual_id + "_reply",
        body           = item_data["body"],
        product_id     = item_data.get("product_id", ""),
        post_type      = item_data["post_type"],
        variant_id     = item_data.get("variant_id", "A"),
    ...
    print(f"手動投稿を記録しました: queue_id={queue_id}")
    return manual_id
...
def run_scheduler(demo: bool = False):
    ...
    if POST_SCHEDULE.get("mode") != "draft":
        ...
        log.info("  draftモードのためエンゲージメント収集は無効化されています")
